...
docker network create --subnet 172.19.0.0/16 --gateway 172.19.0.1 --driver bridge film_network
...
docker run -d --name cms --restart=always -p 3306:3306 -v /www/mysql:/var/lib/mysql -e MYSQL_DATABASE=cms -e MYSQL_USER=cms -e MYSQL_PASSWORD=123456 -e MYSQL_ROOT_PASSWORD=123456 --network=film_network --ip=172.19.0.3 yobasystems/alpine-mariadb:10.11
chmod -R 777 /www
chmod -R 777 /www/cms
